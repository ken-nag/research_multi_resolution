...
class MRUNet_ver2():
    def __init__(self, input_shape, mr_input_shape):
        with tf.variable_scope("U-Net"):
            ...
            self.t_mr_stride = (2,1)
            self.f_mr_stride = (1,2)
            #t 512 to 256
            self.mr_conv1 = Conv2D(8, self.kernel_size, self.t_mr_stride, input_shape=mr_input_shape, padding='same')
            self.mr_Bnorm1 = BatchNormalization()
        
            # endcoder
            self.conv1  = Conv2D(8, self.kernel_size, self.f_mr_stride, input_shape=input_shape, padding='same')
            ...
            self.deconv6  = Conv2DTranspose(1, self.kernel_size, self.f_mr_stride, padding='same')
